...
1. המשתמש בוחר הימור (לוחץ על כפתור).
2. הימור נשמר ב-state.bets.
3. המשתמש לוחץ SPIN.
4. הלקוח שולח POST /api/spin עם { bets, balance }.
5. השרת מבצע:
   - ולידציה
   - RNG (0-36)
   - חישוב זכיות
   - חישוב יתרה חדשה
6. השרת מחזיר JSON Response עם:
   - winningNumber
   - totalBet
   - totalWin
   - newBalance
...
הממצאים המלאים מתועדים ב-SPINOMENAL_RESEARCH.md.
